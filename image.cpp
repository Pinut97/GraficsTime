...
void Image::drawCircle(int a, int b, int r, Color c, bool filled)
{
	int v, x, y;
	x = 0;
	y = r;
...
void Image::drawTriangle(int x0, int y0, int z0, int x1, int y1, int z1, int x2, int y2, int z2, Color c, bool filled)
{
	if (filled) 
	{
		...
		fillTriangle(x0, y0, z0, x1, y1, z1, x2, y2, z2, table, c);
	}
	else 
	{
...
void Image::fillTriangle(int x0, int y0, int z0, int x1, int y1, int z1, int x2, int y2, int z2, std::vector<sCell> table, Color c)
{
	for (int i = 0; i < table.size(); i++)
	{
...
float Image::distanceToCamera(int x0, int y0, int z0, int x1, int y1, int z1, int x2, int y2, int z2, int xp, int yp) 
{
	float mod0 = ((xp - x0) ^ 2 + (yp - y0) ^ 2) ^ (1 / 2);
	float mod1 = ((xp - x1) ^ 2 + (yp - y1) ^ 2) ^ (1 / 2);
	float mod2 = ((xp - x2) ^ 2 + (yp - y2) ^ 2) ^ (1 / 2);

	float mod = mod0 + mod1 + mod2;
	
	float u = mod0 * 10000 / mod;
	float v = mod1 * 10000 / mod;
	float w = mod2 * 10000 / mod;
...
void Image::drawSquare(int x, int y, int size, Color c, bool filled)
{
	if (!filled) 
	{
		DDA(x - size / 2, y + size / 2, x + size / 2, y + size / 2, c);
		DDA(x - size / 2, y - size / 2, x + size / 2, y - size / 2, c);
		DDA(x - size / 2, y - size / 2, x - size / 2, y + size / 2, c);
		DDA(x + size / 2, y - size / 2, x + size / 2, y + size / 2, c);
	}
