...
void Camera::updateViewMatrix()
{
	...
	printf("center: %f, %f, %f   eye: %f, %f, %f   front: %f, %f, %f \n", center.x, center.y, center.z, eye.x, eye.y, eye.z, front.x, front.y, front.z);
	Vector3 side = (front.cross(up)).normalize();
	printf("front: %f, %f, %f   up: %f, %f, %f   side: %f, %f, %f \n", front.x, front.y, front.z, up.x, up.y, up.z, side.x, side.y, side.z);
	Vector3 top = side.cross(front);
	printf("side: %f, %f, %f   front: %f, %f, %f   top: %f, %f, %f \n", side.x, side.y, side.z, front.x, front.y, front.z, top.x, top.y, top.z);

	this->view_matrix.M[0][0] = side.x;
	this->view_matrix.M[1][0] = side.y;
	this->view_matrix.M[2][0] = side.z;
	this->view_matrix.M[0][1] = top.x;
	this->view_matrix.M[1][1] = top.y;
	this->view_matrix.M[2][1] = top.z;
	this->view_matrix.M[0][2] = - front.x;
	this->view_matrix.M[1][2] = - front.y;
	this->view_matrix.M[2][2] = - front.z;
	this->view_matrix.M[3][0] = 0;
	this->view_matrix.M[3][1] = 0;
	this->view_matrix.M[3][2] = 0;
	...
	for (int i = 0; i < 4; i++) 
	{
		printf("%f, %f, %f, %f \n", view_matrix.M[i][0], view_matrix.M[i][1], view_matrix.M[i][2], view_matrix.M[i][3]);
	}
	printf("\nEye: %f, %f, %f \n", eye.x, eye.y, eye.z);
...
void Camera::updateProjectionMatrix()
{
	...
	float f = 1 / std::tan(fov*PI / 180);

	this->projection_matrix.M[0][0] = f / aspect; 
	this->projection_matrix.M[1][1] = f;
	this->projection_matrix.M[2][2] = (far_plane + near_plane)/(near_plane-far_plane); 
